...
JavaScript は動的型付け言語のため、実行されるときに型が動的に決まります。また、型が一致しない場合は、型変換が行われます。したがって、このような型を意識せずに使えることができていました。
...
上記の例で、変数 value は最初に数値型が代入されており、その後文字列型が代入されています。代入する型が変更されていますが、実行時に型変換が行われるため、エラーは発生しません。
